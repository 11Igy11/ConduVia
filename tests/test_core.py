...
class BehaviorProfileTests(unittest.TestCase):
    ...
    def test_activity_profile_html_export_includes_behavior_sections(self):
        ...
        self.assertIn("Activity Profile", content)
        self.assertIn("Case A", content)
        self.assertIn("Case context", content)
        ...
        self.assertNotIn("investigative indicators", content)
        self.assertNotIn("not proof", content)
...
class TableExportTests(unittest.TestCase):
    def test_table_export_html_uses_unified_report_shell(self):
        ...
        self.assertIn("Table data", content)
        ...
        self.assertIn("table-wrap", content)
        self.assertIn("period-badge", content)

    def test_table_export_html_fills_period_and_source(self):
        with temporary_directory() as tmp:
            output = Path(tmp) / "flows.html"
            export_table_html(
                str(output),
                ...
                ["Source", "Volume"],
                [["10.0.0.5", "1.25 MB"]],
                source_label=r"C:\evidence\apple",
                period="29/05/2024 – 29/11/2024",
            )
            content = output.read_text(encoding="utf-8")

        self.assertIn("apple", content)
        self.assertIn("29/05/2024 – 29/11/2024", content)
        self.assertNotIn(r"C:\evidence\apple", content)

    def test_selected_period_label_formats_day_and_range(self):
        from core.exporters.html_blocks import format_export_source_label, format_selected_period_label

        self.assertEqual(format_selected_period_label(active_day="2024-05-29"), "29/05/2024")
        self.assertEqual(
            format_selected_period_label(
                granularity="range",
                range_start="2024-05-29",
                range_end="2024-11-29",
            ),
            "29/05/2024 – 29/11/2024",
        )
        self.assertEqual(format_export_source_label(r"C:\datasets\apple"), "apple")
...
class PcapAnalyzerTests(unittest.TestCase):
    ...
    def test_pcap_html_export_includes_project_context_when_available(self):
        ...
        self.assertIn("PCAP Report", content)
...
class ProfileComparisonTests(unittest.TestCase):
    ...
    def test_interactive_open_guards_large_periods(self):
        ...
        self.assertTrue(should_batch_pcap_files(2, 1024))
        self.assertTrue(should_batch_pcap_files(11, MAX_INTERACTIVE_EVIDENCE_BYTES + 1))
        self.assertFalse(should_batch_pcap_files(1, 1024))
